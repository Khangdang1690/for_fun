...
async def body():
    ...
    with col3:
        st.markdown(
            """
        <div style="padding: 20px; border-radius: 10px; border: 1px solid #ddd; margin-bottom: 20px;">
            <h3>LinkedIn Researcher</h3>
            <p>Finds and enriches LinkedIn profiles by role/company using web search + ContactOut API.</p>
            <p>Perfect for recruiting and business development.</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
        if st.button("Launch LinkedIn Researcher", key="linkedin_researcher_button"):
            st.switch_page("pages/3_LinkedIn_Researcher.py")
